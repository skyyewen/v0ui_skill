...
> 一个类似 v0 的简约 SaaS 商业前端约束 Skill，用来生成现代、干净、可落地的 React / Next.js / Tailwind / shadcn UI。
...
默认视觉目标是简约 SaaS 商业风：白底黑字、浅灰边框、大留白、干净产品面板、顺滑过渡和清晰的人机交互反馈。重要位置可以使用克制的品牌色或状态色。
...
- 希望页面更接近 Vercel 类 SaaS 控制台的白底黑字、干净面板、大留白和顺滑交互风格
...
- 默认使用白底、黑色或近黑文字、浅灰边框和中性色面板，主按钮默认黑色或 `zinc-950`
- 用 token 控制颜色、间距、圆角、边框、阴影和字体层级
- 默认大留白和冷静的信息密度，避免把页面做成拥挤的企业后台
- 卡片和面板默认干净、浅边框、轻阴影、适度圆角；大面板可使用更大的圆角
- 重要位置允许使用克制色彩，例如品牌标识、状态、徽章、图表、进度和选中态
- SaaS、后台、CRM、内部工具默认走简约、商业、可扫描的产品界面，而不是高密度表格优先
- 表单、表格、弹窗、导航、设置项必须包含合理状态和交互反馈
- 避免随机渐变、装饰圆球、嵌套卡片、浓重彩色企业后台风格、单一色系堆叠和文字溢出
- 完成前需要检查桌面端和移动端渲染效果
